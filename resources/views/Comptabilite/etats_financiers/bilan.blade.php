...
<div class="container-fluid financial-report">
    ...
    <header class="text-center mb-3"><h4>{{ $entreprise?->nom_entreprise ?? 'Entreprise' }}@include('partials.entreprise-identifiants')</h4>@if($entreprise?->adresse)<div>{{ $entreprise->adresse }}</div>@endif<h2>Bilan</h2><div>Du {{ \Carbon\Carbon::parse($dateDebut)->format('d/m/Y') }} au {{ \Carbon\Carbon::parse($dateFin)->format('d/m/Y') }} — Devise de présentation : Franc congolais (CDF)</div></header>
