...
<div class="container-fluid financial-report">
    ...
    <header class="text-center mb-3"><h4>{{ $entreprise?->nom_entreprise ?? 'Entreprise' }}</h4>@if($entreprise?->adresse)<div>{{ $entreprise->adresse }}</div>@endif<h2>Bilan final SYSCOHADA</h2><div>Du {{ \Carbon\Carbon::parse($dateDebut)->format('d/m/Y') }} au {{ \Carbon\Carbon::parse($dateFin)->format('d/m/Y') }} — Devise de présentation : Franc congolais (CDF)</div></header>
    ...
    @foreach(['actif'=>'ACTIF','passif'=>'PASSIF'] as $sens=>$titre)
        <div class="col-6"><table class="table table-bordered table-sm"><thead class="table-dark"><tr><th colspan="4">{{ $titre }}</th></tr><tr><th>Réf.</th><th>Libellé</th><th>N</th><th>N-1</th></tr></thead><tbody>
        @foreach($etats['bilan'][$sens] as $section)
            <tr class="table-secondary fw-bold"><td colspan="2">{{ $section['label'] }}</td><td class="text-end">{{ number_format($section['total_actuel'],2,',',' ') }}</td><td class="text-end">{{ number_format($section['total_precedent'],2,',',' ') }}</td></tr>
            @foreach($section['lignes'] as $ligne)<tr><td>{{ $ligne['code'] }}</td><td>{{ $ligne['label'] }}</td><td class="text-end">{{ number_format($ligne['actuel'],2,',',' ') }}</td><td class="text-end">{{ number_format($ligne['precedent'],2,',',' ') }}</td></tr>@endforeach
        @endforeach
        </tbody><tfoot class="table-dark fw-bold"><tr><td colspan="2">TOTAL {{ $titre }}</td><td class="text-end">{{ number_format($etats['bilan']['total_'.$sens],2,',',' ') }}</td><td></td></tr></tfoot></table></div>
    @endforeach
    </div>
...
<style>.financial-report td,.financial-report th{padding:.25rem .4rem}@media print{@page{size:A4 landscape;margin:8mm}.sidebar,.navbar,.no-print{display:none!important}.content{margin:0!important;padding:0!important}.financial-report{font-size:10px}.financial-report .row{display:flex}.financial-report .col-6{width:50%}}</style>
@endsection
