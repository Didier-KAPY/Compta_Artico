<aside class="sidebar" id="sidebar" aria-label="Navigation des ressources humaines">
    @php($estChargeTechnique = auth()->user()->isTechnicalOfficer())
    <div class="px-2 mb-3"><a href="{{ route($estChargeTechnique ? 'parametres.rh.presences' : 'parametres.rh.index') }}" class="d-flex align-items-center gap-2 text-white text-decoration-none px-2"><span class="d-inline-grid rounded-3 bg-info bg-opacity-25 text-center pt-2" style="width:38px;height:38px"><i class="bi bi-person-workspace text-info"></i></span><span><small class="d-block text-white-50">Module</small><strong>Ressources humaines</strong></span></a></div>
    ...
    <ul class="nav flex-column">
        @foreach($liens as [$route,$icone,$libelle,$permission])
            @if((!$estChargeTechnique && (!$permission || auth()->user()->can($permission))) || ($estChargeTechnique && in_array($route, ['presences.scanner', 'presences'], true)))<li><a href="{{ route('parametres.rh.'.$route) }}" class="nav-link {{ request()->routeIs('parametres.rh.'.$route) ? 'active-menu' : '' }}"><i class="bi bi-{{ $icone }} me-2"></i>{{ $libelle }}</a></li>@endif
        @endforeach
        @if(!$estChargeTechnique) @can('manageDepartments')<li><a href="{{route('parametres.departements')}}" class="nav-link {{request()->routeIs('parametres.departements*')?'active-menu':''}}"><i class="bi bi-diagram-3 me-2"></i>Directions et fonctions</a></li>@endcan @endif
