...
<div class="row g-3">@foreach([['Employés',$nombreEmployes,'people','primary'],['Employés actifs',$employesActifs,'person-check','success'],['Directions',$nombreDepartements,'diagram-3','info'],['Fonctions',$nombreFonctions,'briefcase','warning']] as [$libelle,$valeur,$icone,$couleur])<div class="col-sm-6 col-xl-3"><div class="card border-0 shadow-sm h-100"><div class="card-body p-4 d-flex align-items-center gap-3"><span class="rounded-3 bg-{{ $couleur }} bg-opacity-10 text-{{ $couleur }} p-3"><i class="bi bi-{{ $icone }} fs-4"></i></span><div><small class="text-muted">{{ $libelle }}</small><div class="fs-3 fw-bold">{{ $valeur }}</div></div></div></div></div>@endforeach</div>
