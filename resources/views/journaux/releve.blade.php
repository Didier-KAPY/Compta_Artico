...
<div class="container-fluid py-3 sage-screen">
    <div class="sage-titlebar d-flex flex-column flex-lg-row justify-content-between align-items-lg-center gap-3">
        ...
        <div class="d-flex gap-2 no-print"><a href="{{ route('journaux.tresorerie', request()->only('date_debut','date_fin')) }}" class="btn btn-outline-primary"><i class="bi bi-pie-chart me-1"></i> Situation</a><button class="btn btn-light border" onclick="window.print()"><i class="bi bi-printer me-1"></i> Imprimer</button></div>
    </div>
    ...
    <div class="sage-panel mt-3">
        <div class="sage-panel-head"><div><strong>Relevé du {{ \Carbon\Carbon::parse($dateDebut)->format('d/m/Y') }} au {{ \Carbon\Carbon::parse($dateFin)->format('d/m/Y') }}</strong><small>{{ $journaux->count() }} mouvement(s) validé(s)</small></div><span>Montants par devise</span></div>
