...
<div class="container-fluid py-3 sage-screen">
    <div class="sage-titlebar d-flex flex-column flex-lg-row justify-content-between align-items-lg-center gap-3">
        <div><span class="sage-module">Journaux</span><h3><i class="bi bi-list-columns-reverse me-2"></i>Relevé journalier des mouvements</h3><p>Mouvements validés du journal et du compte sélectionnés</p></div>
    ...
    <div class="sage-panel mt-3">
        ...
        <div class="table-responsive"><table class="table sage-grid statement-grid mb-0"><thead><tr><th>Date</th><th>Pièce / Référence</th><th>Journal</th><th>Compte</th><th>Libellé</th><th class="text-end">Entrée CDF</th><th class="text-end">Sortie CDF</th><th class="text-end">Solde CDF</th><th class="text-end">Entrée USD</th><th class="text-end">Sortie USD</th><th class="text-end">Solde USD</th></tr></thead><tbody>
            ...
            <tr><td>{{ $journal->date?->format('d/m/Y') }}</td><td>@can('manageJournaux')<a href="{{ route('journaux.show',$journal->id) }}">{{ $journal->reference }}</a>@else{{ $journal->reference }}@endcan</td><td><span class="journal-code">{{ $journal->journalType?->code ?? '—' }}</span></td><td>{{ $journal->compte?->compte ?? $journal->journalType?->compte?->compte ?? '—' }}</td><td class="description-cell" title="{{ $journal->libelle_releve }}">{{ $journal->libelle_releve }}</td><td class="text-end amount-in">{{ number_format($journal->entrees_cdf,2,',',' ') }}</td><td class="text-end amount-out">{{ number_format($journal->sorties_cdf,2,',',' ') }}</td><td class="text-end fw-semibold">{{ number_format($journal->solde_progressif_cdf,2,',',' ') }}</td><td class="text-end amount-in">{{ number_format($journal->entrees_usd,2,',',' ') }}</td><td class="text-end amount-out">{{ number_format($journal->sorties_usd,2,',',' ') }}</td><td class="text-end fw-semibold">{{ number_format($journal->solde_progressif_usd,2,',',' ') }}</td></tr>
