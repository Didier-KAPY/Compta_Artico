    ...
    <div class="card-body">
        ...
        <form method="POST" action="{{ route('brc.store') }}" enctype="multipart/form-data">@csrf
            <div class="row g-2 mb-3">
                ...
                <div class="col-md-2"><label class="small fw-bold">Sens *</label><select name="sens" class="form-select form-select-sm"><option value="debit" @selected(old('sens') === 'debit')>Débit</option><option value="credit" @selected(old('sens') === 'credit')>Crédit</option></select><small class="text-muted">Sens du journal.</small></div>
                <div class="col-md-2"><label class="small fw-bold">Mode de paiement *</label><select name="mode_paiement" class="form-select form-select-sm" required><option value="mobile_money" @selected(old('mode_paiement', 'mobile_money') === 'mobile_money')>Mobile money</option><option value="espèces" @selected(old('mode_paiement') === 'espèces')>Espèces</option><option value="banque" @selected(old('mode_paiement') === 'banque')>Banque</option></select></div>
            </div>
            <div class="mb-3">
