...
<div class="col-md-6"><label class="form-label">Direction</label><select name="departement_id" class="form-select"><option value="">Tous</option>@foreach($departements as $d)<option value="{{ $d->id }}" @selected((string)old('departement_id',$budget->departement_id)===(string)$d->id)>{{ $d->designation }}</option>@endforeach</select></div>
