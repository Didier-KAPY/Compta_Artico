        ...
        <div class="collapse {{ $errors->any() ? 'show' : '' }}" id="userForm"><div class="card-body p-4">
            <form action="{{ route('profil.user.store') }}" method="POST" enctype="multipart/form-data">@csrf<input type="hidden" name="source" value="parametres">
                <div class="row g-3">
                    ...
                    <div class="col-md-4"><label class="form-label">Direction</label><select name="departement_id" class="form-select"><option value="">Non affecté</option>@foreach($departements as $d)<option value="{{ $d->id }}" @selected(old('departement_id')==$d->id)>{{ $d->designation }}</option>@endforeach</select></div>
