...
<div class="container-fluid py-4 px-lg-4">
    ...
    <div class="card border-0 shadow-sm"><div class="card-header bg-dark text-white py-3 d-flex justify-content-between"><strong>Liste des utilisateurs</strong><span class="badge bg-light text-dark">{{ $users->total() }} compte(s)</span></div><div class="table-responsive"><table class="table table-hover align-middle mb-0"><thead class="table-light"><tr><th>Utilisateur</th><th>Rôle</th><th>Affectation</th><th>Statut</th><th>Dernière connexion</th></tr></thead><tbody>
        @forelse($users as $agent)<tr><td><div class="fw-semibold">{{ $agent->prenom }} {{ $agent->nom }}</div><small class="text-muted">{{ $agent->email }} @if($agent->telephone)· {{ $agent->telephone }}@endif</small></td><td><span class="badge bg-light text-dark border">{{ $agent->role?->designation ?? 'Sans rôle' }}</span></td><td>{{ $agent->departement?->designation ?? '—' }}<small class="d-block text-muted">{{ $agent->fonction?->designation ?? 'Aucune fonction' }}</small></td><td><span class="badge bg-{{ $agent->statut==='Actif'?'success':'secondary' }}">{{ $agent->statut }}</span></td><td>@if($agent->last_logged_in)<span class="fw-semibold">{{ \Carbon\Carbon::parse($agent->last_logged_in)->format('d/m/Y H:i') }}</span><small class="d-block text-muted">{{ \Carbon\Carbon::parse($agent->last_logged_in)->diffForHumans() }}</small>@else<span class="text-muted">Jamais connecté</span>@endif</td></tr>
        @empty<tr><td colspan="5" class="text-center text-muted py-4">Aucun utilisateur.</td></tr>@endforelse
