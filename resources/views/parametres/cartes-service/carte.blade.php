...
<style>
    ...
    .service-card { position:relative; overflow:hidden; width:53.98mm; height:85.60mm; background:#fff; border-radius:3mm; box-shadow:0 10px 30px rgba(12,39,68,.22); color:#152238; font-family:DejaVu Sans, Arial, sans-serif; box-sizing:border-box; }
    .service-card * { box-sizing:border-box; }
    .sc-header { height:19mm; padding:3mm 3mm 2mm; text-align:center; color:#fff; background:linear-gradient(145deg,#0c2948,#1769a5); position:relative; }
    .sc-header:after { content:''; position:absolute; left:0; right:0; bottom:-2mm; height:4mm; background:#e5ad25; transform:skewY(-3deg); transform-origin:left; }
    .sc-logo { width:9mm; height:9mm; border-radius:50%; background:#fff; object-fit:contain; padding:.7mm; vertical-align:middle; float:left; }
    ...
    .sc-sign-line { display:block; border-top:.25mm solid #596673; margin-top:2.7mm; padding-top:.55mm; font-weight:700; }
    ...
    .pdf-card-wrap .service-card { box-shadow:none; border-radius:0; }
    .pdf-card-wrap .sc-header { background:#0c2948; }
    .pdf-card-wrap .sc-header:after { background:#e5ad25; transform:none; bottom:0; height:1.2mm; }
    .pdf-card-wrap .sc-company, .pdf-card-wrap .sc-company strong,
    .pdf-card-wrap .sc-company small, .pdf-card-wrap .sc-title { color:#fff; }
    @media print { body { margin:0 !important; background:#fff !important; } body > * { visibility:hidden; } .service-card-wrap, .service-card-wrap * { visibility:visible; } .service-card-wrap { position:absolute; left:0; top:0; padding:0; } .service-card { box-shadow:none; border-radius:0; } }
</style>
@php
...
<div class="service-card-wrap {{ ($pdfMode ?? false) ? 'pdf-card-wrap' : '' }}">
    <div class="service-card">
        ...
        <div class="sc-bottom"><span class="sc-number">N° {{ $carteService->numero }}</span><div class="sc-sign">Le Gérant<span class="sc-sign-line">{{ $carteService->nom_signataire }}</span></div></div>
    </div>
</div>
@if($pdfMode ?? false)
</body></html>
