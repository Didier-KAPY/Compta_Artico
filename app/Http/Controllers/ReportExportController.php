...
class ReportExportController extends Controller
{
    ...
    public function operationsDiverses(Request $request, string $format)
    {
        ...
        foreach ($query->orderBy('date')->orderBy('id')->get() as $journal) {
            ...
            foreach ($journal->ecritures as $ecriture) {
                ...
                $rows->push([
                    ...
                    $brc?->reference ?? $journal->reference,
                    (float) $ecriture->debit_cdf > 0 ? $compte : '—',
                    (float) $ecriture->credit_cdf > 0 ? $compte : '—',
                    $ecriture->libelle ?: $journal->description ?: '—',
                    (float) $ecriture->debit_cdf > 0 ? $this->montant($montant) : '—',
                    (float) $ecriture->credit_cdf > 0 ? $this->montant($montant) : '—',
        ...
        return $this->telecharger($format, 'Journal des opérations diverses', 'journal-operations-diverses', [
            'Date BRC', 'Validé par', 'Référence BRC', 'Compte débit', 'Compte crédit', 'Libellé',
            'Montant débit', 'Montant crédit', 'Monnaie', 'Statut',
        ], $rows, $request, 'landscape');
    }
    ...
    public function releve(Request $request, string $format)
    {
        ...
        $rows = collect([[$this->date($debut), 'SOLDE D’OUVERTURE', '', '', '', '', '', $this->montant($cdf), '', '', $this->montant($usd)]]);
        foreach ($records as $i) {
            $cdf += (float) $i->entrees_cdf - (float) $i->sorties_cdf;
            $usd += (float) $i->entrees_usd - (float) $i->sorties_usd;
    ...
    public function balance(Request $request, string $format)
    {
        ...
        $data = app(BalanceController::class)->index($request)->getData();
    ...
    {
        $this->autoriser($request, ['Super Admin', 'Admin', 'Directeur Général', 'DAF', 'Comptable']);
        $data = app(EtatFinancierController::class)->bilan($request)->getData();
        $rows = collect();
        foreach (['actif' => 'ACTIF', 'passif' => 'PASSIF'] as $sens => $type) {
            foreach ($data['etats']['bilan'][$sens] as $s) {
                $rows->push([$type, '', $s['label'], $this->montant($s['total_actuel']), $this->montant($s['total_precedent'])]);
                foreach ($s['lignes'] as $l) {
                    $rows->push([$type, $l['code'], $l['label'], $this->montant($l['actuel']), $this->montant($l['precedent'])]);
                }
            }
        }
    ...
    {
        $this->autoriser($request, ['Super Admin', 'Admin', 'Directeur Général', 'DAF', 'Comptable']);
        $data = app(EtatFinancierController::class)->compteResultat($request)->getData();
        $rows = collect();
        foreach (['produits_exploitation', 'charges_exploitation', 'produits_financiers', 'charges_financieres', 'produits_hao', 'charges_hao', 'impot_resultat'] as $key) {
            $s = $data['etats']['compte_resultat'][$key];
            $rows->push(['', $s['label'], $this->montant($s['total_actuel']), $this->montant($s['total_precedent'])]);
            foreach ($s['lignes'] as $l) {
                $rows->push([$l['code'], $l['label'], $this->montant($l['actuel']), $this->montant($l['precedent'])]);
            }
        }
    ...
    private function montant($value): string
    {
        return number_format((float) $value, 2, ',', ' ');
    }

    private function montantOriginalBrc($brc, EcritureComptable $ecriture): float
    {
        if (! $brc) {
            return max((float) $ecriture->debit_cdf, (float) $ecriture->credit_cdf);
        }
        if ((int) $ecriture->liste_des_comptes_id === (int) $brc->journalType?->liste_des_comptes_id) {
            return (float) $brc->total;
        }
