...
class RhPaieWorkflowService {
    public function calculer(Employe $e,int $annee,int $mois,bool $appliquerAbsence,?string $motif,int $auteur,array $extras=[]):RhPaie {
        ...
        return DB::transaction(function()use($e,$annee,$mois,$contrat,$periode,$s,$appliquerAbsence,$motif,$auteur,$extras){
            ...
            $data['salaire_base']=$base;$data['primes']=(float)$contrat->primes_fixes+(float)$contrat->indemnites_fixes+(float)$contrat->prime_fonction+(float)$contrat->prime_responsabilite+(float)($extras['primes']??0);$data['retenues']=$appliquerAbsence?$retenue:0;$data['appliquer_retenue_absence']=$appliquerAbsence;
            $data['avance_salaire'] = $extras['avance_salaire'] ?? 0;
            $data['transport'] = $extras['transport'] ?? $contrat->prime_transport ?? 0;
            $data['logement'] = $extras['logement'] ?? $contrat->prime_logement ?? 0;
            $data['autres_avantages'] = $extras['autres_avantages'] ?? 0;
            $data['telecommunication'] = $extras['telecommunication'] ?? 0;
            $data['periode_paie_id']=$periode->id;
            if ((new RhPaie($data))->calculerNet() < 0) throw ValidationException::withMessages(['avance_salaire'=>'L’avance et les retenues ne peuvent pas dépasser les gains.']);
